...
type
 ...
 { TFWHint = class;
  ...
  TFWAssignedBitmap = class
  private
//    Bitmap: TBitmap;
//    IsImage: Boolean;
  end;

  TFWHintArrows = class(TPersistent)
  private
    FParent: TFWHint;
    FTransparentColor: TColor;
    FLeftTop: TBitmap;
    FRightTop: TBitmap;
    FRightBottom: TBitmap;
    FLeftBottom: TBitmap;
  ...
  published
    constructor Create(AOwner: TFWHint);
    destructor Destroy; override;
    property TransparentColor: TColor read FTransparentColor
      write SetTransparentColor default clWhite;
    property LeftTop: TBitmap index 0 read FLeftTop write SetBitmap;
    property RightTop: TBitmap index 1 read FRightTop write SetBitmap;
    property RightBottom: TBitmap index 2 read FRightBottom write SetBitmap;
    property LeftBottom: TBitmap index 3 read FLeftBottom write SetBitmap;
  end; }
  ...
  published
    ...
    //property CustomArrows: TFWHintArrows read FCustomArrows write FCustomArrows;
...
begin
  ...
  FFont := TFont.Create;
  //FCustomArrows := TFWHintArrows.Create(Self);
end;
...
{procedure TFWHintArrows.AssignTo(Dest: TPersistent);
begin
  inherited;
  AssignTo(Dest);
end;

constructor TFWHintArrows.Create(AOwner: TFWHint);
begin
  if not Assigned(AOwner) then Exit;
  FParent := AOwner;
  FTransparentColor := clWhite;
  FLeftTop := TBitmap.Create;
  FRightTop := TBitmap.Create;
  FRightBottom := TBitmap.Create;
  FLeftBottom := TBitmap.Create;
//  FIsLeftTop := False;
//  FIsRightTop := False;
...
destructor TFWHintArrows.Destroy;
begin
  FLeftTop.Free;
  FRightTop.Free;
  FRightBottom.Free;
...
procedure TFWHintArrows.SetBitmap(const Index: Integer; Bitmap: TBitmap);

  procedure CreateResultBitmap(var Result: TBitmap; X, Y: Byte);
  begin
    with Result do
    begin
      Width := 12;
      Height := 12;
      Canvas.Brush.Color := FParent.FArrowBkColor;
      Canvas.FillRect(Rect(0, 0, 12, 12));
    end;
    BitBlt(Result.Canvas.Handle, X, Y, 12, 12,
      Bitmap.Canvas.Handle, 0, 0, SRCCOPY);
    for Y := 0 to 11 do
      for X := 0 to 11 do
        if Result.Canvas.Pixels[X, Y] = FTransparentColor then
          Result.Canvas.Pixels[X, Y] := FParent.FArrowBkColor;
  end;
...
begin
  ...
  if Assigned(Bitmap) then
    if (Bitmap.Width > 12)
      or (Bitmap.Height > 12) then
      begin
        MessageBox(0, 'Bitmap size must be within 12x12 or smaller.',
        ...
        with Bitmap do
          case Index of
            0: CreateResultBitmap(FLeftTop, 0, 0);
            1: CreateResultBitmap(FRightTop, 12 - Width, 0);
            2: CreateResultBitmap(FRightBottom, 12 - Width, 12 - Height);
            3: CreateResultBitmap(FLeftBottom, 0, 12 - Height);
          end;
end;

procedure TFWHintArrows.SetTransparentColor(const Value: TColor);
var
 // OldColor: TColor;
  X, Y: Byte;
begin
  if not Assigned(FParent) then Exit;
  if Value = FTransparentColor then Exit;
  //OldColor := FTransparentColor;
  FTransparentColor := Value;
  for Y := 0 to 11 do
    for X := 0 to 11 do
    begin
      if FLeftTop.Canvas.Pixels[X, Y] = FTransparentColor then
        FLeftTop.Canvas.Pixels[X, Y] := FParent.FArrowBkColor;
      if FRightTop.Canvas.Pixels[X, Y] = FTransparentColor then
        FRightTop.Canvas.Pixels[X, Y] := FParent.FArrowBkColor;
      if FRightBottom.Canvas.Pixels[X, Y] = FTransparentColor then
        FRightBottom.Canvas.Pixels[X, Y] := FParent.FArrowBkColor;
      if FLeftBottom.Canvas.Pixels[X, Y] = FTransparentColor then
        FLeftBottom.Canvas.Pixels[X, Y] := FParent.FArrowBkColor;
    end;
end;  }
